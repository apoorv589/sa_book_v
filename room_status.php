...
<?php
    ...
    if($checked_app) {
        $table = $ui->table()
            ...
            <tr class="bg-success">
                <td><a href="'.site_url('sah_booking/booking_request/details/'.$checked_app['app_num'].'/'.$auth).'">'.$checked_app['app_num'].'</a></td>
    ...
    $table = $ui->table()
        ...
        foreach($room_bookings as $booking) {
            echo '
            <tr class="bg-success">
                <td><a href="'.site_url('sah_booking/booking_request/details/'.$booking['app_num'].'/'.$auth).'">'.$booking['app_num'].'</a></td>
                <td>'.$booking['name'].'</td>
            </tr>';
        }
    $table->close();
