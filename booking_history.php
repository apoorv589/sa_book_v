...
?>
						<tr>
							<td><a href="<?php echo site_url("sah_booking/booking_request/details/".$data_array_approved[$sno][1]."/".$auth);?>"><?php echo $data_array_approved[$sno][1];?></a></td>
							<td><?php echo $data_array_approved[$sno][2];?></td>
							<td><?php echo $data_array_approved[$sno][3];?></td>
							<td><? if($data_array_approved[$sno][4]['hod_status'] == 'Cancel' || $data_array_approved[$sno][4]['dsw_status'] == 'Cancel' || $data_array_approved[$sno][4]['est_ar_status'] == 'Cancel') echo '<center>'.$ui->icon('clock-o').'Cancellation Pending</center>'; 
							else if($data_array_approved[$sno][4]['est_ar_status'] == 'Cancelled') echo 'Cancellation Approved'; 
							else if($data_array_approved[$sno]['guest_checked_in'] != 0) echo '<center>'.$ui->icon('remove').'Cancel Not Permitted</center>';
							else {?><center>
							<? $form = $ui->form()
									   ->multipart()
									   ->action('sah_booking/booking_request/cancellation/'.$data_array_approved[$sno][1].'/'.$this->session->userdata('auth')[0])
									   ->open();
...
?>
						<tr>
							<td><a href="<?php echo site_url("sah_booking/booking_request/details/".$data_array_rejected[$sno][1]."/".$auth);?>"><?php echo $data_array_rejected[$sno][1];?></a></td>
...
?>
						<tr>
							<td><a href="<?php echo site_url("sah_booking/booking_request/details/".$data_array_cancelled[$sno][1]."/".$auth);?>"><?php echo $data_array_cancelled[$sno][1];?></a></td>
