<?php
	...
	$row = $ui->row()->open();
		...
		if ((($auth != 'stu' && $auth != 'emp' && strlen($est_ar_status) != 0) || (($auth == 'stu' || $auth == 'emp') && ($ctk_allotment_status == 'Pending' || strlen($est_ar_status) != 0))) && $hod_status != 'Rejected' && $dsw_status != 'Rejected') {
		    if($est_ar_status == 'Approved')
		    	$status = 'Approved : '.date('d/m/Y H:m', strtotime($est_ar_action_timestamp));
			else if($est_ar_status == '')
				$status = 'Pending';
			else $status = $est_ar_status;
		    if($est_ar_status == 'Approved'){
		    	$ui->input()
				   ->width(3)
				   ->uiType('success')
				   ->type('text')
				   ->label('<i class="fa fa-clock-o"></i> EST AR Status')
				   ->value($status)
				   ->disabled()
			       ->show();
		    }
		...
		if ($deny_reason) if($deny_reason != 'NULL' && $deny_reason != NULL) {
			if($est_ar_status != 'Cancelled' && $hod_status != 'Cancel' && $dsw_status != 'Cancel' && $est_ar_status != 'Cancel')
	...
	$row = $ui->row()->open();
		if($no_of_rooms == 0 && $ctk_allotment_status == 'Approved') {
			...
			$col = $ui->col()
				...
				foreach($rooms as $room)
					echo ucfirst($room['building']).' - '.ucfirst($room['floor']).' - '.$room['room_no'].' - '.$room['room_type'].'<br/>';
			$col->close();
		}
		...
		if($school_guest == '1')
		{
			$col = $ui->col()->width(6)->open();
				...
				echo '<a href="'.site_url('../assets/files/sah_booking/'.$user_id.'/'.$file_path).'">Click to view</a>';
			$col->close();
		}
	$row->close();

	if(($est_ar_status != 'Cancelled') &&
		(($auth == 'hod' || $auth == 'hos') && $hod_status == 'Pending') ||
		($auth == 'dsw' && $dsw_status == 'Pending') ||
		($auth == 'est_ar' && $est_ar_status == 'Pending'))
	{
		echo '<br/>';
		//reject approve and submit buttons
		$form = $ui->form()->action('sah_booking/booking_request/official_action/'.$app_num.'/'.$auth)->open();
	...
	if((($auth == 'hod' || $auth == 'hos') && $hod_status == 'Cancel') || ($auth == 'dsw' && $dsw_status == 'Cancel') || ($auth == 'est_ar' && $est_ar_status == 'Cancel'))
	{
		$form = $ui->form()->id('cancel_form')->action('sah_booking/booking_request/cancellation/'.$app_num.'/'.$auth)->open();
